...
namespace SfSoft.BLL
{
	...
	{
		private readonly IDRP_Invite_My_Arts dal=DataAccess.CreateDRP_Invite_My_Arts();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public SfSoft.Model.DRP_Invite_My_Arts GetModelByCache(int ID)
		{
			
			string CacheKey = "DRP_Invite_My_ArtsModel-" + ID;
			...
			return (SfSoft.Model.DRP_Invite_My_Arts)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<SfSoft.Model.DRP_Invite_My_Arts> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<SfSoft.Model.DRP_Invite_My_Arts> DataTableToList(DataTable dt)
		{
			List<SfSoft.Model.DRP_Invite_My_Arts> modelList = new List<SfSoft.Model.DRP_Invite_My_Arts>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				SfSoft.Model.DRP_Invite_My_Arts model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new SfSoft.Model.DRP_Invite_My_Arts();
					...
					if(dt.Rows[n]["IsSales"]!=null && dt.Rows[n]["IsSales"].ToString()!="")
					{
						model.IsSales=int.Parse(dt.Rows[n]["IsSales"].ToString());
					}
					if(dt.Rows[n]["IsContact"]!=null && dt.Rows[n]["IsContact"].ToString()!="")
					{
						model.IsContact=int.Parse(dt.Rows[n]["IsContact"].ToString());
					}
					if(dt.Rows[n]["IsJoin"]!=null && dt.Rows[n]["IsJoin"].ToString()!="")
					{
						model.IsJoin=int.Parse(dt.Rows[n]["IsJoin"].ToString());
					}
