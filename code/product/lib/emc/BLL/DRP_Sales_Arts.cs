...
namespace SfSoft.BLL
{
	...
	{
		private readonly IDRP_Sales_Arts dal=DataAccess.CreateDRP_Sales_Arts();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public SfSoft.Model.DRP_Sales_Arts GetModelByCache(int ID)
		{
			
			string CacheKey = "DRP_Sales_ArtsModel-" + ID;
			...
			return (SfSoft.Model.DRP_Sales_Arts)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<SfSoft.Model.DRP_Sales_Arts> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<SfSoft.Model.DRP_Sales_Arts> DataTableToList(DataTable dt)
		{
			List<SfSoft.Model.DRP_Sales_Arts> modelList = new List<SfSoft.Model.DRP_Sales_Arts>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				SfSoft.Model.DRP_Sales_Arts model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new SfSoft.Model.DRP_Sales_Arts();
